...
class BB84(Protocol):
    ...
    def transmittance_i_photon_state_x(self, i):
        ...
        return (
            ...
            - (
                ...
                * self.receiver.x_basis_transmittance()
                * self.channel.transmittance()
                * self.source.optical_efficiency()
            )
            ** i
        )

    def yield_i_photon_state_x(self, i):
        return self.detector.background_rate() + self.transmittance_i_photon_state_x(
            i
        ) * (1 + self.detector.after_pulsing)

    def gain_i_photon_state_x(self, i):
        return self.yield_i_photon_state_x(i) * self.source.probability_sending_i_state(
            i
        )
    ...
    def quantum_bit_error_rate_x(self, i):
        return (
            1 / 2 * self.detector.background_rate()
            + (self.channel.detection_error + 1 / 2 * self.detector.after_pulsing)
            * self.transmittance_i_photon_state_x(i)
        ) / self.yield_i_photon_state_x(i)

    def overall_quantum_bit_error_rate_x(self):
        qber = 0
        for i in range(0, 50):
            qber += (
                self.quantum_bit_error_rate_x(i)
                * self.yield_i_photon_state_x(i)
                * self.source.probability_sending_i_state(i)
            )
    ...
    def transmittance_i_photon_state_z(self, i):
        ...
        return (
            ...
            - (
                1
                - self.detector.efficiency
                * self.receiver.z_basis_transmittance()
                * self.channel.transmittance()
                * self.source.optical_efficiency()
            )
            ** i
    ...
    def yield_i_photon_state_z(self, i):
        return self.detector.background_rate() + self.transmittance_i_photon_state_z(
            i
        ) * (1 + self.detector.after_pulsing)

    def gain_i_photon_state_z(self, i):
        return self.yield_i_photon_state_z(i) * self.source.probability_sending_i_state(
            i
        )
    ...
    def quantum_bit_error_rate_z(self, i):
        return (
            1 / 2 * self.detector.background_rate()
            + (self.channel.detection_error + 1 / 2 * self.detector.after_pulsing)
            * self.transmittance_i_photon_state_z(i)
        ) / self.yield_i_photon_state_z(i)

    def overall_quantum_bit_error_rate_z(self):
        qber = 0
        for i in range(0, 50):
            qber += (
                self.quantum_bit_error_rate_z(i)
                * self.yield_i_photon_state_z(i)
                * self.source.probability_sending_i_state(i)
            )
    ...
    def key_rate_decoy_state_inf_key(self):
        return (
            self.source.probability_sending_i_state(0) * self.detector.background_rate()
            + self.source.probability_sending_i_state(1)
            * (self.yield_i_photon_state_x(1) + self.yield_i_photon_state_z(1))
            / 2
            * (1 - binary_shannon_entropy(self.quantum_bit_error_rate_x(1)))
            - self.overall_gain_z()
            * self.correction_efficiency
            * binary_shannon_entropy(self.overall_quantum_bit_error_rate_z())
        )
    ...
    def key_rate_no_decoy_state_inf_key(self):
        ...
        delta = (
            1
            - self.source.probability_sending_i_state(0)
            - self.source.probability_sending_i_state(1)
        ) / self.gain()
        ...
        return self.gain() * (
            ...
            * (
                ...
                - binary_shannon_entropy(
                    self.overall_quantum_bit_error_rate_x() / (1 - delta)
                )
            )
            - self.correction_efficiency
            * binary_shannon_entropy(self.overall_quantum_bit_error_rate_z())
...
class Pulsed_BBM92(Protocol):
    ...
    def __init__(
        ...
        *,
        source: Source,
        detector1: Detector,
        channel_1: FiberChannel,
        receiver1: Receiver,
        correction_efficiency: float,
        detector2: Optional[Detector] = None,
        channel_2: Optional[FiberChannel] = None,
        receiver2: Optional[Receiver] = None,
    ):
    ...
    def transmittance_i_photon_state1_Z(self, i):
        ...
        return (
            ...
            - (
                1
                - self.detector1.efficiency
                * self.receiver1.z_basis_transmittance()
                * self.channel_1.transmittance()
                * self.source.optical_efficiency()
                * (1 + self.detector1.after_pulsing)
            )
            ** i
        )
    ...
    def transmittance_i_photon_state2_Z(self, i):
        ...
        return (
            ...
            - (
                1
                - self.detector2.efficiency
                * self.receiver2.z_basis_transmittance()
                * self.channel_2.transmittance()
                * self.source.optical_efficiency()
                * (1 + self.detector2.after_pulsing)
            )
            ** i
        )
    ...
    def yield_i_photon_stateZ(self, i):
        ...
        return (
            1
            - (1 - self.detector1.background_rate())
            * (1 - self.transmittance_i_photon_state1_Z(i))
        ) * (
            1
            - (1 - self.detector2.background_rate())
            * (1 - self.transmittance_i_photon_state2_Z(i))
        )

    def gain_i_photon_stateZ(self, i):

        return self.yield_i_photon_stateZ(i) * self.source.probability_sending_i_state(
            i
        )
    ...
    def entanglement_errorZ(self, n, m):
        ...
        return 1 / 2 - (
            (
                ...
                - (
                    (
                        self.channel_1.detection_error
                        + self.channel_2.detection_error
            ...
            / self.yield_i_photon_stateZ(n)
        ) * (
            -self.transmittance_i_photon_state1_Z(n - m)
            + self.transmittance_i_photon_state1_Z(m)
        ) * (
            -self.transmittance_i_photon_state2_Z(n - m)
            + self.transmittance_i_photon_state2_Z(m)
        )
    ...
    def overall_quantum_bit_error_rateZ(self):
        ...
        for i in range(0, 50):
            qber += (
                self.quantum_bit_error_rateZ(i)
                * self.yield_i_photon_stateZ(i)
                * self.source.probability_sending_i_state(i)
            )
    ...
    def transmittance_i_photon_state1_X(self, i):
        ...
        return (
            ...
            - (
                1
                - self.detector1.efficiency
                * self.receiver1.x_basis_transmittance()
                * self.channel_1.transmittance()
                * self.source.optical_efficiency()
                * (1 + self.detector1.after_pulsing)
            )
    ...
    def transmittance_i_photon_state2_X(self, i):
        ...
        return (
            ...
            - (
                1
                - self.detector2.efficiency
                * self.receiver2.x_basis_transmittance()
                * self.channel_2.transmittance()
                * self.source.optical_efficiency()
                * (1 + self.detector2.after_pulsing)
            )
    ...
    def yield_i_photon_stateX(self, i):
        ...
        return (
            1
            - (1 - self.detector1.background_rate())
            * (1 - self.transmittance_i_photon_state1_X(i))
        ) * (
            1
            - (1 - self.detector2.background_rate())
            * (1 - self.transmittance_i_photon_state2_X(i))
        )

    def gain_i_photon_stateX(self, i):

        return self.yield_i_photon_stateX(i) * self.source.probability_sending_i_state(
            i
        )
    ...
    def entanglement_errorX(self, n, m):
        ...
        return 1 / 2 - (
            ...
            / self.yield_i_photon_stateX(n)
        ) * (
            -self.transmittance_i_photon_state1_X(n - m)
            + self.transmittance_i_photon_state1_X(m)
        ) * (
            -self.transmittance_i_photon_state2_X(n - m)
            + self.transmittance_i_photon_state2_X(m)
        )
    ...
    def overall_quantum_bit_error_rateX(self):
        ...
        for i in range(0, 50):
            qber += (
                self.quantum_bit_error_rateX(i)
                * self.yield_i_photon_stateX(i)
                * self.source.probability_sending_i_state(i)
            )
    ...
    def key_rate(self):
        ...
        return (self.final_gain() / 2) * (
            1
            - binary_shannon_entropy(self.overall_quantum_bit_error_rateX())
            - binary_shannon_entropy(self.overall_quantum_bit_error_rateZ())
            * self.correction_efficiency
        )
...
class BBM92_continuous_wave_pumped_source(Protocol):
    ...
    def true_coincidence_rate_i_n_x(self, i, n):
        return (
            self.heralding_efficiency_1_x()
            * self.heralding_efficiency_2_x()
            * no_x_event_i_time(self.heralding_efficiency_1_x(), i - 1)
            * no_x_event_i_time(self.heralding_efficiency_2_x(), i - 1)
            * no_x_event_i_time(self.heralding_efficiency_2_x() / 2, n - i)
            * no_x_event_i_time(self.heralding_efficiency_1_x() / 2, n - i)
            * no_x_event_i_time(self.detector1.background_rate() / 2, 1)
            * no_x_event_i_time(self.detector2.background_rate() / 2, 1)
        )
    ...
    def true_coincidence_rate_x(self):
        sum = 0
        for i in range(1, 5):
            sum = sum + self.true_coincidence_rate_x_i(
                i
            ) * self.source.probability_sending_i_state(i, self.coincidence_time)
        return sum / self.coincidence_time

    def miss_coincidence_x_i(self, i):
        return (
            no_x_event_i_time(self.heralding_efficiency_1_x(), i)
            + no_x_event_i_time(self.heralding_efficiency_2_x(), i)
            - no_x_event_i_time(self.heralding_efficiency_1_x(), i)
            * no_x_event_i_time(self.heralding_efficiency_2_x(), i)
            - no_x_event_i_time(self.heralding_efficiency_1_x(), i)
            * (1 - no_x_event_i_time(self.heralding_efficiency_2_x(), i))
            * self.detector1.background_rate()
            - no_x_event_i_time(self.heralding_efficiency_2_x(), i)
            * (1 - no_x_event_i_time(self.heralding_efficiency_1_x(), i))
            * self.detector2.background_rate()
            - no_x_event_i_time(self.heralding_efficiency_1_x(), i)
            * no_x_event_i_time(self.heralding_efficiency_2_x(), i)
            * self.detector1.background_rate()
            * self.detector2.background_rate()
    ...
    def miss_coincidence_x(self):
        sum = 0
        for i in range(1, 5):
            sum = sum + self.miss_coincidence_x_i(
                i
            ) * self.source.probability_sending_i_state(i, self.coincidence_time)
        return sum
    ...
    def accidental_coincidence_rate_x(self):
        ...
        return (
            1
            - self.true_coincidence_rate_x() * self.coincidence_time
            - self.miss_coincidence_x()
            - self.source.probability_sending_i_state(0, self.coincidence_time)
            * (1 - self.detector1.background_rate() * self.detector2.background_rate())
        ) / self.coincidence_time

    def measured_coincidence_rate_x(self):

        return (
            self.source.coincidence_window_efficiency(self.coincidence_time)
            * self.true_coincidence_rate_x()
            + self.accidental_coincidence_rate_x()
        )
    ...
    def coincidence_error_rate_x(self):
        ...
        return (
            self.source.coincidence_window_efficiency(self.coincidence_time)
            * self.true_coincidence_rate_x()
            * self.overall_detector_error()
            + self.accidental_coincidence_rate_x() / 2
        )
    ...
    def true_coincidence_rate_i_n_z(self, i, n):
        return (
            self.heralding_efficiency_1_z()
            * self.heralding_efficiency_2_z()
            * no_x_event_i_time(self.heralding_efficiency_1_z(), i - 1)
            * no_x_event_i_time(self.heralding_efficiency_2_z(), i - 1)
            * no_x_event_i_time(self.heralding_efficiency_2_z() / 2, n - i)
            * no_x_event_i_time(self.heralding_efficiency_1_z() / 2, n - i)
            * no_x_event_i_time(self.detector1.background_rate() / 2, 1)
            * no_x_event_i_time(self.detector2.background_rate() / 2, 1)
        )
    ...
    def true_coincidence_rate_z(self):
        sum = 0
        for i in range(1, 5):
            sum = sum + self.true_coincidence_rate_z_i(
                i
            ) * self.source.probability_sending_i_state(i, self.coincidence_time)
        return sum / self.coincidence_time
    ...
    def miss_coincidence_z_i(self, i):
        return (
            no_x_event_i_time(self.heralding_efficiency_1_z(), i)
            + no_x_event_i_time(self.heralding_efficiency_2_z(), i)
            - no_x_event_i_time(self.heralding_efficiency_1_z(), i)
            * no_x_event_i_time(self.heralding_efficiency_2_z(), i)
            - no_x_event_i_time(self.heralding_efficiency_1_z(), i)
            * (1 - no_x_event_i_time(self.heralding_efficiency_2_z(), i))
            * self.detector1.background_rate()
            - no_x_event_i_time(self.heralding_efficiency_2_z(), i)
            * (1 - no_x_event_i_time(self.heralding_efficiency_1_z(), i))
            * self.detector2.background_rate()
            - no_x_event_i_time(self.heralding_efficiency_1_z(), i)
            * no_x_event_i_time(self.heralding_efficiency_2_z(), i)
            * self.detector1.background_rate()
    ...
    def miss_coincidence_z(self):
        sum = 0
        for i in range(1, 5):
            sum = sum + self.miss_coincidence_z_i(
                i
            ) * self.source.probability_sending_i_state(i, self.coincidence_time)
        return sum
    ...
    def accidental_coincidence_rate_z(self):
        ...
        return (
            1
            - self.true_coincidence_rate_z() * self.coincidence_time
            - self.miss_coincidence_z()
            - self.source.probability_sending_i_state(0, self.coincidence_time)
            * (1 - self.detector1.background_rate() * self.detector2.background_rate())
        ) / self.coincidence_time
    ...
    def measured_coincidence_rate_z(self):
        ...
        return (
            self.source.coincidence_window_efficiency(self.coincidence_time)
            * self.true_coincidence_rate_z()
            + self.accidental_coincidence_rate_z()
        )
    ...
    def coincidence_error_rate_z(self):
        ...
        return (
            ...
            * self.true_coincidence_rate_z()
            * self.overall_detector_error()
            + self.accidental_coincidence_rate_z() / 2
        )
    ...
    def overall_measured_coincidence(self):
        ...
        return (
            self.measured_coincidence_rate_z() + self.measured_coincidence_rate_x()
        ) / 2

    def key_rate(self):
        return (self.overall_measured_coincidence() / 2) * (
            1
            - binary_shannon_entropy(self.qber_x())
            - self.correction_efficiency * binary_shannon_entropy(self.qber_z())
        )
...
class Continuous_Entanglement_swapping(Protocol):
    ...
    def __init__(
        ...
        *,
        ...
        self.covariance_matrix = self.source_1.emitted_pairs_covariance_matrix(
            bell_measurement_number + 1
        )
    ...
    def beam_splitter_matrix(self, t):
        ...
        return np.kron(
            ...
            scipy.linalg.block_diag(
                np.eye(2),
                np.kron(np.eye(self.bell_measurement_number), partial_matrix),
                np.eye(2),
            ),
        )
    ...
    def beam_splitter_entanglement(self):
        ...
        return np.dot(
            self.beam_splitter_matrix.T,
            np.dot(self.covariance_matrix, self.beam_splitter_matrix),
        )
    ...
    def polarizers_matrix(self, polarizer_angle_1, polarizer_angle_2):
        ...
        A = np.array([[a_1, a_2], [-a_2, a_1]])

        b_1 = np.cos(polarizer_angle_2)
        b_2 = np.sin(polarizer_angle_2)

        B = np.array([[b_1, b_2], [-b_2, b_1]])

        return np.kron(
            np.eye(2),
            scipy.linalg.block_diag(A, np.eye(4 * self.bell_measurement_number), B),
        )
    ...
    def losses_matrix(self):
        ...
        n = (
            self.source_1.optical_efficiency()
            * self.detector_1.efficiency
            * self.channel_1.transmittance()
            * self.receiver_1.transmittance
            * self.receiver_1.x_basis_transmittance()
        )
    ...
    def apply_losses_matrix(self, polarizer_angle_1, polarizer_angle_2):
        ...
        return (
            np.dot(
                K.T,
                np.dot(self.apply_polarizer(polarizer_angle_1, polarizer_angle_2), K),
            )
            + A
        )
