...
def slant_range_from_zenith_angle(
    observer_alt: float, target_alt: float, zenith_angle: float
) -> float:
    ...
    return float(channel_length)


def compute_minimum_alt(length: float, height: float) -> float:
    """Compute minimum altitude of a horizontal channel between two aerial platforms at
    the same height.
...
def compute_sec(theta: float) -> float:
...
def compute_central_angle(
    lat_1: float, lon_1: float, lat_2: float, lon_2: float
) -> float:
    """Compute central angle between two points on the Earth's surface given their
    ...
    angle = np.arccos(
        ...
        + np.cos(phi_1) * np.cos(phi_2) * np.cos(delta_lambda)
    )
    angle_deg = np.rad2deg(angle)
    return float(angle_deg)


def slant_range_from_central_angle(
    alt_1: float, alt_2: float, central_angle: float
) -> float:
    ...
    channel_length = np.sqrt(
        eff_alt_1**2
        + eff_alt_2**2
        - 2 * eff_alt_1 * eff_alt_2 * np.cos(np.deg2rad(central_angle))
    )
    return float(channel_length)
...
def zenith_angle_from_slant_range(
    slant_range: float, observer_alt: float, target_alt: float
) -> float:
    ...
    zenith_angle_deg = np.rad2deg(zenith_angle)
...
def compute_observer_azimuth(
    ...
    denominator = math.cos(phi_obs) * math.sin(phi_target) - math.sin(
        phi_obs
    ) * math.cos(phi_target) * math.cos(d_lam)
