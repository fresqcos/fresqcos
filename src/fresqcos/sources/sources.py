...
class Multiplexed_Heralded_Photon_Source(Source):
    ...
    def probability_sending_i_state(self, i):
        ...
        if i == 0:
            ...
            return (
                poisson.pmf(i, self.mean_photon_number)
                * (1 - np.exp(-self.mean_photon_number * self.sources_num))
...
class Symmetric_Multiplexed_Heralded_Photon_Source(Source):
    ...
    def sources_num(self, value: int) -> None:
        if value <= 0 or (value & (value - 1)) != 0:
            raise ValueError(
                f"sources_num must be non-negative and a power of 2  , got {value}"
            )
        self._sources_num = int(value)
    ...
    def probability_sending_i_state(self, i):
        ...
        return (1 - self.efficiency) * np.exp(
            ...
            -self.efficiency * self.mean_photon_number * (2 / self.transmittance) ** k
        ) / math.factorial(
            i
        ) + poisson.pmf(
            i, self.mean_photon_number
        ) * (
            1
            - ((1 - self.efficiency) ** i)
            * np.exp(
            ...
            - np.exp(
                -self.efficiency
                * self.mean_photon_number
                * (2 / self.transmittance) ** k
            )
        ) / (
            1
            - np.exp(
                -self.efficiency * self.mean_photon_number / (self.transmittance**k)
            )
        )
        ...
        for k in range(1, self.sources_num):
            ...
            sum += (
                np.exp(
                    -self.efficiency
                    * self.mean_photon_number
                    * ((self.transmittance ** (1 - k) - 1) / (1 - self.transmittance))
                )
                * (1 - ((1 - self.efficiency) ** k))
                * np.exp(
                    self.efficiency * self.mean_photon_number
                    - self.efficiency * self.mean_photon_number / self.transmittance**k
                )
            )

        return poisson.pmf(i, self.mean_photon_number) * sum + (
            1 - self.efficiency
        ) * np.exp(-(1 - self.efficiency) * self.mean_photon_number) * np.exp(
            -self.efficiency
            * self.mean_photon_number
            * (
                (
                    (2 - self.transmittance)
                    * self.transmittance ** (1 - self.sources_num)
                )
                - 1
            )
            / (1 - self.transmittance)
        ) / math.factorial(
            i
        )
...
class Single_Photon_Source(Source):
    ...
    def brightness(self, value: float) -> None:
        if value <= 0 or value > 1:
            raise ValueError(
                f"brightness must be non-negative and less than 1, got {value}"
            )
    ...
    def probability_sending_i_state(self, i):
        if i < 0 or i > 2:
            ...
            p2 = (
                1
                - self.g2 * self.brightness
                - np.sqrt(1 - 2 * self.g2 * self.brightness)
            ) / self.g2
...
class Sagnac_Sources(Source):
    ...
    def two_modes_squeezed_vacuum_states(self, sources_number):
        ...
        partial_x_quadrature_matrix = np.array([[c_1, c_2], [c_2, c_1]])

        partial_p_quadrature_matrix = np.array([[c_1, -c_2], [-c_2, c_1]])
    ...
    def emitted_pairs_covariance_matrix(self, sources_number):
        ...
        partial_swapping_matrix = np.array(
        ...
        return np.dot(
            swapping_matrix.T,
            np.dot(
                self.two_modes_squeezed_vacuum_states(sources_number), swapping_matrix
            ),
        )
